...
Rustit therefore owns an open project model with stable identities for BIM objects, schedule objects, and their relationships. Desktop authoring, 4D visualization, database persistence, APIs, vendor synchronization, and AI all operate through the same deterministic model.
...
5. **Vendors are adapters.** IFC, P6, Oracle Primavera Cloud, Bentley SYNCHRO, and future systems connect at defined boundaries.
6. **Geometry is replaceable.** A geometry kernel is an implementation behind an abstraction, not the architecture of the application.
7. **The model is headless.** Core operations work without launching the desktop UI.
8. **Operations become auditable.** Model and schedule changes will be proposed, validated, diffed, committed, and reversible.
9. **AI uses the same API.** AI may propose transactions but never bypasses validation, policy, or review.
10. **Open interoperability is a product.** IFC and documented APIs are first-class engineering work, not export checkboxes.
